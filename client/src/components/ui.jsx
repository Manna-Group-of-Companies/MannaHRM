...
export function Modal({ title, actions, foot, why, extra, wide, msg, onClose }) {
	return (
		<div
			className="modal"
			role="dialog"
		...
		>
			<div className={"box" + (wide ? " wide" : "")}>
				...
				{/* **The footer is where a dialog's own button goes**, beside Close and
				    outside the scroll. It used to be the last thing inside `.mscroll`
				    on the two forms that have one, which put Save under whatever the
				    form happened to be tall enough to hide — and a Save somebody has
				    to scroll to find is a dialog that looks stuck. That is the same
				    finding the `.mscroll` note above records, one control along.

				    `foot` rather than the existing `actions`: that one is a toolbar
				    at the top of the body, which is a different place and a different
				    job. */}
				<div className="foot">
					{foot || null}
...
/* Why a control that opens the desk is drawn dead: the desk address has not
   been resolved yet. It is settled during the first load, so this is a moment
   at startup rather than a state anybody sits in — but a link with no href
   still looks live, and a live-looking control that does nothing is the thing
   every dead button on this dashboard was written to avoid. */
export const NO_SITE = "Which desk this opens has not been settled yet — this opens there once it has.";
...
/** A control that does its job on the ERPNext site instead of here.
    ...
    with `dead` saying why — when there is not. Nothing here calls the site: a
    link needs no session and no allowlist. See `lib/desk.js`.
...
/** Their caret menu: **Data import from file** and **Download template**.
    ...
    `onImport` overrides that: a screen that can read a file itself passes one,
    and the item becomes a button opening its own picker rather than a link out
    to the site. `importWhy` is what that item then says, because the standing
    reason on `items` is written for the link and would be a lie on the button.
    A screen with no picker of its own passes neither and gets the link, which
    is what Calendar still does.
...
export function ImportMenu({
	open, onToggle, onClose, href, label, title, items,
	onImport, importWhy, importDead, onTemplate, templateWhy, templateDead,
}) {
	return (
		<span className="empdrop">
			...
			<div className="emmenu end catimpmenu" role="menu" aria-label="Import" hidden={!open}>
				{items.map(([k, ico, text, why]) => {
					...
					if (k === "template") {
						return (
							<button key={k} role="menuitem" disabled={!onTemplate}
								title={onTemplate ? (templateWhy || why) : templateDead}
					...
					/* A file picker on this side, where the screen has one. It needs no
					   site to be reachable and no `href` at all: the reading happens
					   here, and the write it may then make is the caller's business.

					   `importDead` alone is a screen that has a picker in principle and
					   nothing for it to read on this row — drawn disabled with that
					   reason, rather than falling through to a link that would open the
					   wizard on a doctype the screen has just said does not exist. */
					if (onImport || importDead) {
						return (
							<button key={k} role="menuitem" disabled={!onImport}
								title={onImport ? (importWhy || why) : importDead}
								onClick={(e) => { e.stopPropagation(); onImport(); onClose(); }}>
								{mark}
								{text}
							</button>
						);
