...
/** A file that is already on the server, saved rather than opened.
    ...
    cross-origin, which is why `/files` and `/private` are proxied onto :5173 in
    development (see `server.proxy` in client/vite.config.js).

    `?download=1` is the site's own switch — the same URL without it renders
