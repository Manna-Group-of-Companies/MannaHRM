...
function SrForm({ s }) {
	...
	function button(k) {
		...
		if (k === "close") return go({ srRun: false, srMsg: "", subtab: "all" });
		if (k === "pick") return patch("sr", { pick: !f.pick });
		if (k === "export") return srExport(s);
		/* This printed the paragraph below and did nothing else, until Factor HR's
		   own Auto Report Scheduler was photographed on this report — the same
		   single form Employee Detail opens, down to the five buttons. It opens
		   that now, for this report.

		   Not one word of the paragraph has stopped being true: the schedule is
		   still created on the site, by ERPNext's Auto Email Report, and it still
		   needs an outgoing mail account there to send anything. What the form
		   buys is that the questions are Factor HR's and the typing happens once
		   — and the dialog says all of that on itself rather than here. */
		if (k === "schedule") {
			openScheduler("stat");
			return set({
				srMsg: "<b>The schedule is created on the site.</b> Frappe’s <em>Auto Email Report</em> sends "
					+ "a saved report on a cron to a list of people, which is exactly what this dialog fills "
					+ "in — and it still needs the one thing this page cannot supply: an outgoing mail account "
					+ "on the bench. Worth knowing the day somebody asks who receives the PF return each month.",
			});
		}
		if (k === "background") {
			set({
...
export default function Statutory() {
	...
	return (
		<>
			...
			{/* Factor HR's Auto Report Scheduler, opened for this report — the same
			    single form Employee Detail's ⏰ Schedule Report opens, keyed by which
			    report asked for it. See data/schedreport.js, where the five reports
			    and the two dialogs they divide between are written down. */}
			{s.sched.open ? (
