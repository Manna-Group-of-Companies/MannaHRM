		...
		<>
					...
					<div className="fhtitle row">
						...
						<span className="ics">
							...
							<Desk href={s.site && deskImport(s.site)} label="Import"
								title="Imports leave from a spreadsheet. Opens ERPNext's Data Import on the site, which previews the file before it writes — this page proxies GET only, see server/index.js.">
								<svg viewBox="0 0 24 24" width="17" height="17" stroke="currentColor" fill="none"
								...
								onChange={(e) => put({
									...
									msg: e.target.files?.[0]
										? `<b>${e.target.files[0].name} was read by this browser and goes no further.</b> `
											...
											+ "to the application; this page proxies GET only — see <code>server/index.js</code>."
										: "",
								})} />
							<span className="hint">
			...
			<Panel title="Apply Leave, field by field" cov="part" ico="🗂">
				...
				<NoteBelow>
					...
					these fields. Until the proxy's allowlist widens (<code>server/index.js</code>), an application
