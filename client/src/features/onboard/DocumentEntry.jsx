...
import { DOC_COLS, DOC_KINDS, DOC_PAGE, DOC_REGISTERS, DOC_RELATED_ORDER, DOC_VIEW, FH_DOCS } from "@/data/onboard";
import { Desk, Modal, Scroll, panelProps, tabProps } from "@/components/ui";
...
function DocView({ r, s }) {
	...
	return (
		<>
			...
			{/* Under their form rather than in it, because the scan is not on their
			    dialog — it is on their *row*. Keeping it out of the boxes above is
			    the difference between drawing their screen and editing it.

			    Their row also records who filed the document and when, and nothing on
			    this side is behind either. Both sat here as "to build" rows and are
			    gone: a panel of what does not exist yet is noise to the person
			    reading a record. An unbuilt field belongs on the register's coverage
			    chips, where somebody is asking what is covered. */}
			<div className="dvmore">
				<h5>The scan filed against this document</h5>
				<div className="rows">
					<div className="row">
						<span>
							Attachment{" "}
...
/* ---------------------------------------------------------------------------
   ...
   **This is the one screen on this dashboard that writes, and the exception is
   ...
   the five document fields on `Employee`, and the attachment calls are the only place
...
function DocEdit({ r, s, onClose, making }) {
	...
	return (
		<div className="deform">
			{/* **What to do first, where somebody is looking when they wonder.**

			    New opens this dialog with six greyed boxes, because a document on
			    this side is a *field on a person* — so until an employee and a type
			    are picked there is no field for a number to be written to. That was
			    said only twice: in a tooltip on a disabled Save, and in small print
			    beside it. Reported from the running app as "clicking New, no data,
			    unable to type", which is what a form of greyed boxes with the
			    explanation at the bottom looks like. */}
			{making && !row ? (
				<div className="denote">
					<b>Pick an employee and a document type first.</b> The six boxes below stay off until
					both are chosen — a document here is a field on a person, so those two decide which
					field the number is written to.
				</div>
			) : null}
			...
			<div className="dvgrid">
				<div className="dvbox">
					...
					{/* **Their three, drawn as three.** This was a one-option select
					    saying Employee, which hid the finding rather than showing it:
					    the whole point of the register above is that they keep
					    documents against dependants and against the company too, and
					    this side can hold neither. Their capture of the open dropdown
					    (5 Sep 2026) is Company / Employee / Dependant.

					    Options rather than a note, and unselectable rather than
					    absent — the same bargain Document Type makes below. A choice
					    drawn and disabled says "this exists there and not here"; a
					    choice left out says nothing at all. See DOC_REGISTERS for
					    what each would need. */}
					<select id="de-rel" className="dectl" value="employee" disabled={busy}
						title="Which register the document goes in. Theirs has three; two of them have nothing on this site to hang a document off."
						onChange={() => {}}>
						{DOC_RELATED_ORDER
							.map((key) => DOC_REGISTERS.find((r) => r.key === key))
							.filter(Boolean)
							.map((reg) => (
							<option key={reg.key} value={reg.key} disabled={reg.key !== "employee"}
								title={reg.why}>
								{reg.label}{reg.key === "employee" ? "" : " — nothing on this site holds one"}
							</option>
							))}
					</select>
				</div>
				...
				<div className="dvbox">
					...
					<input id="de-no" className="dectl mono" value={form.no} disabled={busy || !row}
						/* `kind` is `{}` until a type is picked, so this said "Writes
						   undefined on this employee." on every freshly opened New —
						   the one box somebody tries first, explaining itself in a word
						   that means nothing to anybody. */
						title={kind.num
							? `Writes ${kind.num} on this employee.`
							: making
								? "Pick a document type above. It decides which field on Employee the number is written to."
								: "No field on this site holds a number for this document type."}
