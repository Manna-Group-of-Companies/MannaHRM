...
export default function AssetEntry() {
	...
	return (
		<div className="afform">
			...
			{/* **Why the boxes are not taking typing, in the place somebody is
			    looking when they wonder.**

			    Only the submitted case was said here, and the other two are the
			    ones people actually meet first: the form draws thirteen boxes
			    before anything is picked, and a picked draft still needs Edit
			    pressing. Both were explained only by a tooltip on a button
			    nobody had reason to hover — so the form was tried first, took
			    the cursor, and swallowed the keystrokes. Reported from the
			    running app as "unable to type and save", which is exactly what
			    that is.

			    A disabled button still explains itself on hover. This is for the
			    people who never get that far. */}
			{!busyForm ? (
				<div className="afnote">
					{submitted ? (
						<>
							<b>Submitted — read only.</b> This asset is at docstatus 1, so its record is
							history and neither this form nor the site will let it be typed into. A draft one
							can be. Open it on the site to amend it, where that act has an approval behind it.
						</>
					) : asset ? (
						<>
							<b>Read only — press Edit to type.</b> The boxes show what the record holds until
							then. Nine of the thirteen can be changed here; the greyed four have no field
							behind them on ERPNext&rsquo;s Asset, and status and custodian are moved by an
							Asset Movement rather than typed.
						</>
					) : (
						<>
							<b>No asset picked.</b> Press <b>New</b> for a blank one, or <b>Search</b> to put a
							record from the register into the boxes. Nothing here takes typing until one of
							those has been done.
						</>
					)}
				</div>
