...
/* ---------------------------------------------------------------------------
   ...
   **Three of the seven columns are newer than this dashboard's first read of
   ...
   `Employee Letter`, and a site whose copy of that doctype predates them
   refuses the whole read rather than returning them blank — so `load()` falls back to the four columns that have always been
...
const MAIL_DEAD = "Their envelope mails the selected letters out of the product. Nothing here can send mail: "
	+ "there is no mail path here, and a mailto: cannot carry the letters as attachments. "
			...
			<header>
				...
				<span className="right">
					...
					{/* Theirs opens Create New Letter, and so does this: the form
					    ...
					    page while it is up. Its Save writes the document from here —
					    which is what makes this the shorter way in, and why New Letter
					    Document was taken off this bar. */}
					<button className="embtn pri" onClick={onCreate}
						title="Open Create New Letter — their form, with the employee, the letter type, the date, a reference number and remarks. Save writes the Employee Letter on the site and this register is re-read; Cancel comes back here.">
						Create Letter
					</button>
				</span>
			</header>
