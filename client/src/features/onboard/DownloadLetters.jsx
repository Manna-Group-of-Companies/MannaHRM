	...
	async function run() {
		...
		try {
			/* The text is not on the list read — a register does not carry the
			   ...
			   rather than all at once: forty parallel reads is how a dashboard gets
			   itself rate-limited — the site has a daily compute limit and answers
			   429 when it is reached — and the count on the button is only honest
			   if the requests are ordered. */
			const full = [];
