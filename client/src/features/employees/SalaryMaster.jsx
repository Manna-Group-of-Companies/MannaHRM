...
/* Factor HR's Salary Master, photographed 29 August 2026: the title with three
   ...
   What this screen cannot do is show a revision: no salary doctype is read by
   this dashboard, and none of them holds a row on the site anyway. It is
...
const PickRow = ({ e }) => (
	<button onClick={() => {
		patch("sal", { emp: e.name, q: "", list: false });
		/* Picking somebody lands on their revisions, never on a half-typed form
		   belonging to the person before them. The draft itself survives — it is
		   kept per person (see `rev.by`) — so their + brings it straight back. */
		patch("rev", { open: false, msg: "" });
	}}>
		...
		<>
			...
			<div className="fhscreen">
				<div className="fhtitle row">
					...
					<span className="ics">
						{/* Locking a master against edits is a permission, not a switch — on
						    ...
						    payroll doctype is read by this dashboard. */}
				...
				<div className="embar salbar">
					...
					{/* Their + opens the revision form on whoever is picked, and so does
					    this one — every wage type they carry, drawn in SalaryRevision.jsx.

					    It used to be a link to the site, and the link has not been lost:
					    it moved inside the form, onto the SAVE it belongs to. That is the
					    better place for it. A revision is a dated change to what somebody
					    is paid and of every write this dashboard does not make it is the
					    one it should be least able to make — but sending somebody to the
					    site before they have seen what the form even asks for answered a
					    question nobody had got to yet.

					    It opens with nobody picked, and theirs does not. A revision still
					    belongs to one person — the form takes nothing until it knows whose
					    pay it is — but refusing to *draw* it until somebody is chosen
					    answers "what does a pay revision even ask for?" with a greyed-out
					    button. That is the question this whole page exists to answer, and
					    it is asked most often by somebody who has not picked anybody yet.
					    So the form opens and asks who it is for at its own top. */}
					<button className="embtn ic" aria-label="Add salary revision"
						aria-pressed={s.rev.open}
						title={emp
							? `Open the salary revision form for ${emp.employee_name} — every wage type Factor HR carries. Nothing on it writes; the SAVE buttons at its foot say why.`
							: "Open the salary revision form — every wage type Factor HR carries. It asks who it is for at its top; nothing on it writes."}
						onClick={() => patch("rev", { open: !s.rev.open, msg: "" })}>
						+
					</button>
				...
				{/* Their List of Employees: nine columns, a filter under each, and their
				    pager. Drawn whether or not somebody is already picked — theirs is a
				    way of changing who that is, and a list that vanished the moment it
				    worked would be a list you could never use twice. */}
				{s.sal.list ? (
					<EmployeeList
						pool={pool}
						...
						onPick={(e) => {
							patch("sal", { emp: e.name, q: "", list: false });
							/* Same as PickRow above: land on their revisions, never on a
							   half-typed form belonging to the person before them. */
							patch("rev", { open: false, msg: "", err: "", done: null });
						}} />
				) : null}
				...
				{!emp ? (
					...
					<>
						...
						{/* A second way into the form, from the one place on the page that
						    says there is nothing to see. The bar's + is theirs and stays
						    where theirs is; this block is already ours, and a person told
						    "no revisions" should not have to go looking for the control
						    that makes one. */}
						<div className="mt-[.7rem]">
							<button className="btn imp"
								title={`Open the salary revision form for ${emp.employee_name}. It is Factor HR's, wage type for wage type — and nothing on it writes.`}
								onClick={() => patch("rev", { open: true, msg: "" })}>
								＋ Add a revision
							</button>
						</div>
					</>
