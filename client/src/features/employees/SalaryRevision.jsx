...
function Saved({ s, emp, done, later }) {
	return (
		<div className="mt-[.8rem]">
			<Note>
				<b>Written as drafts.</b> Nobody is paid from these until a human submits them on
				the site — this page cannot, and the proxy refuses any <code>docstatus</code> but 0.
				<ul className="srplan">
					<li>
						<code>Salary Structure</code>{" "}
						<Desk label="structure" className="lk"
							href={s.site && deskUrl(s.site, "Salary Structure", done.structure)}
							title="Opens this structure on the ERPNext site, in a new tab.">
							{done.structure} ↗
						</Desk>
					</li>
					<li>
						<code>Salary Structure Assignment</code>{" "}
						<Desk label="assignment" className="lk"
							href={s.site && deskUrl(s.site, "Salary Structure Assignment", done.assignment)}
							title="Opens this assignment on the ERPNext site, in a new tab. Submitting it there is what makes it payroll.">
							{done.assignment} ↗
						</Desk>{" "}
						· {emp.employee_name} · from {done.on}
					</li>
					{done.created.length ? (
						<li>
							<b>{done.created.length}</b> Salary Component
							{done.created.length === 1 ? "" : "s"} created, because the site had none by
							that name: {done.created.join(", ")}
						</li>
					) : null}
					{done.reused.length ? (
						<li>{done.reused.length} existing component{done.reused.length === 1 ? "" : "s"} reused: {done.reused.join(", ")}</li>
					) : null}
					{done.skipped.length ? (
						<li>
							Not written: {done.skipped.map((k) => k.desc).join(", ")} — a total is derived
							by hrms rather than stored, and writing one as a component counts the same
							money twice.
						</li>
					) : null}
				</ul>

				{/* Their second button's claim, answered honestly. */}
				{later ? (
					later.drafts.length + later.submitted.length === 0 ? (
						<p className="mt-[.5rem]">
							No revision for {emp.employee_name} is dated after {done.on}, so there was
							nothing later to update.
						</p>
					) : (
						<p className="mt-[.5rem]">
							<b>
								{later.drafts.length + later.submitted.length} later revision
								{later.drafts.length + later.submitted.length === 1 ? "" : "s"} found, and
								this page did not touch {later.drafts.length + later.submitted.length === 1 ? "it" : "them"}.
							</b>{" "}
							{later.submitted.length
								? `${later.submitted.length} of them ${later.submitted.length === 1 ? "is" : "are"} submitted — amending one changes the basis of pay somebody may already have been given, which wants a human and an approval rather than a button. `
								: ""}
							Open them on the site:{" "}
							{[...later.drafts, ...later.submitted].map((r) => (
								<Desk key={r.name} label={r.name} className="lk"
									href={s.site && deskUrl(s.site, "Salary Structure Assignment", r.name)}
									title={`${r.docstatus === 1 ? "Submitted" : "Draft"}, from ${r.from_date}`}>
									{r.name} ↗{" "}
								</Desk>
							))}
						</p>
				...
				<button className="btn ghost mt-[.6rem]" onClick={() => patch("rev", { done: null, later: null })}>
					Dismiss
				</button>
			</Note>
		</div>
...
export default function SalaryRevision({ emp }) {
	...
	return (
		<div className="srform">
			...
			{pressed && !busy && !s.rev.done ? (
				<div className="mt-[.8rem]">
					<Note>
						<b>{pressed.label} — this is what it will write.</b>{" "}
						<Html html={pressed.why} />
						<Plan plan={planRevision(d)} />
						<div className="mt-[.6rem] flex gap-2 flex-wrap">
							<button className="btn imp" onClick={() => write(pressed.k)}>
								Write {pressed.k === "future" ? "the drafts and check later dates" : "the drafts"}
							</button>
							<button className="btn ghost" onClick={() => patch("rev", { msg: "" })}>Cancel</button>
							<Desk className="btn ghost" label="Open on the site"
								href={s.site && deskNew(s.site, "Salary Structure Assignment")}
								title="Opens a blank Salary Structure Assignment on the ERPNext site, if this is better done by hand. What is typed here stays where it is — this opens in a new tab.">
								↗ Do it on the site instead
							</Desk>
						</div>
					</Note>
				</div>
			...
			{s.rev.err ? (
				<div className="mt-[.8rem]">
					<Note>
						<b>The site refused this.</b> Nothing was submitted, and nobody is paid from a
						draft.
						<pre className="srerr">{s.rev.err}</pre>
						<button className="btn ghost" onClick={() => patch("rev", { err: "" })}>Dismiss</button>
					</Note>
				</div>
			) : null}
			...
			{s.rev.msg === "clear" && !ro ? (
				<div className="mt-[.8rem]">
					<Note>
						<b>
							Discard {touched.length} row{touched.length === 1 ? "" : "s"} typed for{" "}
							{emp.employee_name}?
						</b>{" "}
						Nothing on this form has been saved anywhere, so this is the only copy. Export first if
						the figures are wanted.
						<div className="mt-[.6rem] flex gap-2 flex-wrap">
							<button className="btn ghost" onClick={() => revCsv(emp, d)}>⬇ Export first</button>
							<button className="btn imp" onClick={clear}>Discard them</button>
							<button className="btn ghost" onClick={() => patch("rev", { msg: "" })}>Keep them</button>
						</div>
					</Note>
				</div>
			) : null}
			...
			<div className="mt-4">
				...
				<div className="mt-[.7rem]">
					<Note>
						<b>Their one form is our two documents and a master.</b> A revision here is a{" "}
						<code>Salary Structure</code> holding the components and their formulae, plus a{" "}
						<code>Salary Structure Assignment</code> naming the person and the date it takes effect
						from. Theirs carries the amounts on the person, so every employee is their own
						structure; ours is meant to share one structure across everybody on the same terms.{" "}
						<b>Which of the two shapes the group actually runs on is the first payroll question,
						and it is still open</b> — it decides whether a pay revision is one document or a
						hundred and sixty.
					</Note>
				</div>
			</div>
