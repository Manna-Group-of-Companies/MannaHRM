		...
		<>
			<div className="mt-4">
				<Panel title={`${t.l} — what a request carries`} cov={t.cov} ico={t.ico}>
					...
					<NoteBelow>
						Source: <code>{t.src}</code> <Html html={t.maps} />
					</NoteBelow>
				</Panel>
			</div>
			...
			{t.tools && (
				<div className="mt-4">
					<Panel title={`${t.l} — what the queue does`} cov="part" ico="🧰">
						...
						<NoteBelow>
							{t.view === "grid"
								? "This queue has its own toolbar, as it does in Factor HR. "
								: "The toolbar is shared by every tab, so this list holds for all seven. "}
							{READ_ONLY}
						</NoteBelow>
					</Panel>
				</div>
	...
	return (
		<>
			...
			<Note>
				...
				<b>Leave and Attendance are read from the site; the other five are field lists awaiting your
				call</b> — Factor HR holds nothing in any of them either, so this is a decision about what to
				build rather than a backlog to clear.
			</Note>
		</>
	);
}
