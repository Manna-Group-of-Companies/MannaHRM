...
/* ---------------------------------------------------------------------------
   **SCHEDULE REPORT LIST** — behind View Scheduled Reports in the same menu,
   ...
   The rows come from `Auto Email Report`, which ships with Frappe and is read
   here — so an empty table almost certainly means nobody has set a schedule up,
   rather than a doctype nobody can reach.
...
export const SREP_LIST_COLS = [
	...
	{ key: "act", label: "Action", state: "part",
		why: "Edit opens the schedule on the site, which is the only place it can be changed. Delete is "
			+ "drawn and dead: removing a schedule decides who stops receiving a report, and it is made "
