...
export const DOC_KINDS = [
	/* ------------------------------------------------------------------------
	   **Their seven, captured from the open dropdown on 5 September 2026**, in
	   their order — National Id, Visa, Contract, Resident Card, Man Power Id,
	   Insurance Card, Passport. Before this only three were known, because the
	   list had never been seen open; the code said so and guessed low.

	   The shape of the finding got worse rather than better. One of their seven
	   has a field on this site. **Four of the six that do not are UAE
	   paperwork** — Visa, Resident Card, Man Power Id, Insurance Card — and
	   `Manna Tyre UAE` is a real company on this site (SITE_SURVEY.md §2), so
	   these are not hypothetical types on a menu nobody uses. They are the
	   documents that decide whether somebody may legally be at work, each with a
	   renewal date behind it, and this side can hold none of them.

	   That makes the Document register a **harder** blocker than the eleven rows
	   in their export suggested, and it is worth reading alongside
	   OPEN_QUESTIONS.md §4: the UAE company already needed a decision before the
	   Employee master is loaded, and this is a second reason for the same
	   conversation rather than a new one.

	   PAN is last and is ours, not theirs — it is not on their list at all, and
	...
	},
	{
		...
		why: "A work visa, with an expiry that decides whether somebody may legally be at work. Nothing on "
			+ "ERPNext's Employee holds one — not the number and not the date. This is the sharpest of the "
			+ "six gaps: a passport that has expired is an inconvenience, and a visa that has is a person "
			+ "who cannot be on site tomorrow. See OPEN_QUESTIONS.md §4.",
	},
	{
		key: "contract", label: "Contract", state: "build",
		why: "The employment contract. **Half of this one exists**: ERPNext's Employee carries "
			+ "<code>contract_end_date</code>, which is on the New Employee form here already — so the "
			+ "renewal date has somewhere to live and the contract *number* does not. A register keyed on "
			+ "a number cannot draw a row from a date alone, which is why this is dead rather than partial.",
	},
	{
		key: "resident", label: "Resident Card", state: "build",
		why: "The UAE residence card — Emirates ID in everything but name. No field on this site, and the "
			+ "same renewal cycle as the visa it is issued against, so the two expire together and neither "
			+ "is being watched from here.",
	},
	{
		key: "manpower", label: "Man Power Id", state: "build",
		why: "Their term for the labour-card or work-permit id, which is issued per worker per employer. "
			+ "Nothing on this site holds it. It is the one on this list most likely to be asked for by an "
...
/** The order their **Related To** dropdown lists the three registers in,
    captured 5 September 2026: Company, Employee, Dependant.

    Deliberately not the order of `DOC_REGISTERS` below, which is the order the
    three registers are stacked down their *page* — Employee, Dependant,
    Company. Their own product disagrees with itself about the order, and both
    are copied rather than one being tidied into the other: which of the two a
    reader is looking at is the thing that decides which order is right. */
export const DOC_RELATED_ORDER = ["company", "employee", "dependant"];
...
/** Which of the thirteen boxes can be typed into.
    ...
    **The site is what decides, not this list.** A control enforced in a browser
    is a suggestion to anyone holding curl (CLAUDE.md §1), so what a person may
    actually change on an `Asset` is their ERPNext roles and whether the document
    has been submitted — a submitted asset refuses every field not marked
    allow-on-submit, with 417. This list only stops the form offering a box that
    would come back refused; when the two disagree, the site wins and the box
    goes red on Save rather than quietly doing nothing.
