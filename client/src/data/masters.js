...
/* The three buttons and the Data Import menu, in Factor HR's order. All of them
   write, and nothing here writes a `Holiday List` — so rather than sitting
   dead with the reason on a tooltip, each one opens the same job on the ERPNext
   site, where a holiday list is a document and where the rules that guard it
   actually run. A link rather than a form on purpose: a calendar decides who is
   expected at the gate, and that judgement belongs on the desk with the
   validation and the version trail. See CLAUDE.md §1.
...
export const CTC_RATING_MENU = [
  /* The reason here has been restated twice as the architecture moved, and the
     conclusion has not changed once. It is no longer true that this page cannot
     write — it can, as the signed-in person, under their own roles. The accurate
     reason is the stronger one and worth stating properly rather than softening:
     a spreadsheet of pay figures is the wrong shape for this write, whoever is
     allowed to make it. */
  ["upload", "⬆", "Upload Rating from Excel file",
   ...
   + "pay somebody the wrong amount without anybody noticing. Every write here goes in as you and under "
   + "your own roles, which is exactly why this one is not offered: a file picker that rewrites a pay "
   + "field on hundreds of records has no preview, no per-row refusal and nothing to read back. "
   + "ERPNext&rsquo;s own Data Import has all three, and <code>ctc</code> wants a rule on the site in "
...
/* ---------------------------------------------------------------------------
   ...
   Category drill behind it, and both items work on both — which they did not
   when this note was first written.

   **Import reads the file here.** It was a link to ERPNext's Data Import, and
   the reason was sound: a bulk write belongs where the validation is. It still
   does, and the link is still on the dialog's footer — but the dialog reads the
   sheet on this side first and says what every row would do before anything is
   sent. See `lib/catsheet.js` for what it decides and
   `features/employees/CategoryImport.jsx` for the screen.

   **Download template writes what the site holds**, on the drill for the one
   master it is showing and on the list across all three at once — the
   `Category Type` column is what lets one file carry three. It used to be dead
   on the list, on the grounds that a template of category types is a file with
   no doctype to load into. That was true of a template *of category types* and
   is not true of this one: it is a template of their **values**, each row
   naming the master it belongs to, which is exactly what the reader on the
   other side of the menu wants.
   --------------------------------------------------------------------------- */
...
export const IMPORT_MENU = [
	["import", "up", "Data import from file",
		...
		+ "belongs, and why a screen with no reader of its own sends you there rather than pretending "
		+ "to do it in a browser tab."],
...
/** What Import says on Categories, where it opens a picker here rather than the
    wizard on the site. Overrides the standing reason on IMPORT_MENU, which is
    written for the link — see `ImportMenu`'s `importWhy`. */
export const CAT_IMPORT_WHY = "Reads a CSV here and says what every row would do before anything is "
	+ "written: which values are new, which the site already holds, and which cannot be written from "
	+ "this side at all. Only the new ones are created, as you, under your own roles. ERPNext's own "
	+ "Data Import is one click further on, on the dialog's footer, and is still the way to update "
	+ "something that already exists.";

/** And what Download template says there. The list writes all three masters at
    once and the drill writes the one it is showing, which is the only
    difference between the two — so the reason is one string. */
export const CAT_TEMPLATE_WHY = "The values the site already holds, in the columns the import above "
	+ "reads — so a new value is a row added at the bottom rather than a list retyped. The ID column is "
	+ "Frappe's own: a row carrying one is a record that already exists and is skipped, and a row with "
	+ "it left blank is the new one.";
...
/* Their two buttons. Both write, and neither writes from here — no payroll
   screen on this dashboard has ever been wired to. So each says what it would
   do and where it can actually be done, which is the same bargain every other
   write on this dashboard makes.
...
/* ---------------------------------------------------------------------------
   **New** on the Employees calendar — Factor HR's create screen, photographed
   ...
   Everything else on this dashboard that cannot write is stopped because no
   screen was wired to write it. This is stopped for a reason of its own, and it
   is worth keeping when somebody is tempted to wire it up — a holiday list
   decides who is expected at the gate, and the toolbar on the view screen has
   said so since it was built.
...
/* ---------------------------------------------------------------------------
   ...
   on a toolbar, and it is the site's own link validation that makes it.
