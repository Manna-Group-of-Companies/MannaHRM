...
/* **The roles. The values are in `src/styles/themes.css`.**
   ...
   Every colour entry below resolves to a custom property, so this file names
   what a colour is *for* and the theme file decides what it is. That is the
   whole reason a restyle is one file and not four hundred edits: the stylesheet
   names roles.

   One palette ships — Manna, the logo's orange on a near-black navy rail — in
   two modes, light and dark. Five others shipped alongside it until 5 September
   2026 on a second `data-theme` axis; the note at the top of `themes.css`
   records what they were and how to bring the axis back.
   ...
   **No `darkMode` here, deliberately.** Tailwind's own dark variant would mean
   writing every rule twice — `bg-card dark:bg-slate-900` on four hundred class
   lists. The tokens already carry the mode: `--card` *is* the dark surface under
   `[data-mode="dark"]`, so `bg-card` is right in both and nothing needs a
   variant. The one thing that trap catches is a literal — `bg-white` was 63 of
   them, and they are all `bg-card` now. */
export default {
  ...
  theme: {
    extend: {
      /* `<alpha-value>` is what keeps `bg-brand-soft/60` and `text-bad/70`
         ...
         hand-written CSS. Use `rgb(var(--x))` there instead. */
      colors: {
        ...
        /* `ink-3` names every micro-label, column head and count in the app, so
           ...
           clears 4.5:1 on white, on `wash`, on `page` and on `rule` — in both
           modes, and `npm run contrast` is what proves it. */
        ink: {
        ...
        bad: {
          ...
          /* **The fill, as against the colour.** `bad` is the red somebody
             *reads* — a refusal, a failing count, a chip. In dark mode that has
             to be a light red, and white text on a light red is unreadable, so
             the destructive button needed a token that stays mid-tone in both
             modes. In all six light palettes `bad-fill` is `bad` exactly, so
             nothing moved on screen when it was introduced. See the DARK block
             in themes.css, and WHITE_ON in scripts/contrast.mjs. */
          fill: "rgb(var(--bad-fill) / <alpha-value>)",
      ...
      boxShadow: {
        ...
        /* **The values moved to `themes.css` when dark mode arrived**, for the
           same reason the colours did: ink at 5% alpha is depth on a white page
           and nothing at all on a near-black one, so the four steps have to be
           able to differ per mode. This file still names them; the theme file
           still decides what they are. */
        raise: "var(--sh-raise)",
        lift: "var(--sh-lift)",
        pop: "var(--sh-pop)",
        modal: "var(--sh-modal)",
