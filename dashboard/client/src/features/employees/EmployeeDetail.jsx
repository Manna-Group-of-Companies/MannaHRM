...
import DownloadDocs from "@/features/employees/DownloadDocs";
import ScheduleReport, { openScheduler } from "@/features/employees/ScheduleReport";
import ScheduleList from "@/features/attendance/ScheduleList";
		...
		<>
			...
			<div className="repform">
				...
				<div className="repacts">
					...
					{/* Their Download Document dialog — six kinds and two Generates.

					    This was a link to the desk's File list, with a note saying
					    attachments were not read through this proxy. **The note had gone
					    stale**: `File` is on the server's allowlist, `/files/<name>`
					    serves the bytes under a sandbox CSP, and On Board's Document
					    register has been opening them for weeks. The desk is still one
					    click away, from inside the dialog. See DownloadDocs.jsx. */}
					<button className="btn ghost"
						title="Their Download Document dialog: profile pictures, identity scans, or everything filed against a person. Particular Employee above narrows it to one; empty covers everybody."
						onClick={() => set({ dl: { ...s.dl, open: true, msg: "", bad: false } })}>
					...
					{/* Their Auto Report Scheduler — fifteen controls, six of which have
					    a field on ERPNext's Auto Email Report and seven of which do not.
					    See data/schedule.js, which is where that table lives.

					    This was a bare `deskNew` link to that doctype, and the hand-off
					    has not changed: scheduling needs something running when nobody is
					    watching and this page is a browser tab, so Create/Update Schedule
					    inside the dialog opens the site's own scheduler. What the dialog
					    adds is that the questions are Factor HR's, the form arrives
					    filled in, and the seven answers the site has nowhere to put are
					    named rather than lost on the way. */}
					<button className="btn ghost"
						title="Their Auto Report Scheduler. The schedule is created on the ERPNext site — nothing here runs when nobody is watching — but the form is theirs and it opens the site's own already filled in."
						onClick={() => openScheduler("ed")}>
						⏰ Schedule Report
					</button>
			...
			{s.dl.open ? (
				<DownloadDocs onClose={() => set({ dl: { ...s.dl, open: false } })} />
			) : null}

			{s.sched.open ? (
				<ScheduleReport onClose={() => set({ sched: { ...s.sched, open: false } })} />
			) : null}

			{/* The scheduler's ▤ List opens Factor HR's list screen rather than the
			    site's, so this page has to be able to draw it. Same dialog, same
			    state, as on the three attendance reports. */}
			{s.sreplist.open ? (
