...
export default defineConfig({
  ...
  server: {
    port: 5173,
    /* `/files` as well as `/api`: the bytes behind an attachment are fetched by
       the browser itself — a tab, a save — so they have to look like they came
       from :5173 the same way every API call does. Without this the Document
       register's paperclip opens Vite's own 404 page. */
    proxy: {
      "/api": { target: `http://localhost:${PROXY_PORT}`, changeOrigin: true },
      "/files": { target: `http://localhost:${PROXY_PORT}`, changeOrigin: true },
    },
  },
  build: {
