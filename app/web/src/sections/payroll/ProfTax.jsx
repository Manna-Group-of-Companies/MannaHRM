...
function PtForm({ s }) {
	...
	function button(k) {
		...
		if (k === "close") return set({ ptRun: false, ptMsg: "", subtab: "overview" });
		...
		<>
			...
			<Cols>
				<Panel title="Who this applies to" cov="live" ico="🗺">
					{s.companies.length ? (
						<>
							...
							<NoteBelow>
								Read live off <code>Company</code>. <b>The split is the finding.</b> One salary
								structure cannot serve both sides of it — the UAE company deducts none of the
								Indian statutory heads, and its payroll is a different regulator's problem
								entirely. See F4, which is the same company causing the same trouble for
								timezones.
							</NoteBelow>
						</>
					) : (
				...
				<Panel title="What their form asks and this site cannot answer" cov="none" ico="🧾">
					...
					<NoteBelow>
						<b>State is the one that has to be settled before payroll rather than during it.</b> PT
						is levied state by state — the slab, the return, the due date and the registration all
						differ — so &ldquo;which state is this person taxed in&rdquo; has to have an answer on
						the record before a single deduction is computed. Their form filters by it, which means
						their master holds it. Ours does not, and the group does not sit in one state. It is a
						field on <code>Employee</code> or a value derived from the branch, and either way it is
